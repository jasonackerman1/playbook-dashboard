...
<!-- ── Roster ──────────────────────────────────────────────────────────── -->
<div class="section">
  <div class="section-header" style="display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:12px;">
    <div>
      <div class="section-title">Certification Roster <span class="info-btn" onclick="showInfo(event,'roster')">?</span></div>
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end;">
      <select id="f-mgr" class="filter-select" onchange="applyFilters()"><option value="">All Managers</option></select>
      <input type="text" id="f-search" class="roster-search" placeholder="Search name&hellip;" oninput="applyFilters()" style="width:160px;">
    </div>
  </div>
...
(function(){{
  ...
  var mgrs = [...new Set(PEOPLE.map(function(p){{ return p.Manager; }}).filter(Boolean))].sort();
  mgrs.forEach(function(m){{
    sel("f-mgr").innerHTML += '<option value="' + m + '">' + m + "</option>";
  }});
}})();
...
function applyFilters(){{
  ...
  filtered = PEOPLE.filter(function(p){{
    ...
    if(mgr && p.Manager !== mgr) return false;
    if(q){{
