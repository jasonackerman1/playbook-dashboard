...
function runExport(type) {{
  ...
  if(type==='full') {{
    ...
    sel('print-s1-label').textContent=`LMS COMPLETE — AWAITING CERTIFICATION (${{almostThere.length}})`;
    ...
    sel('print-s2-label').textContent=`CERTIFIED — MISSING SECONDARY CURRICULUM (${{secGaps.length}})`;
    sel('print-roster-head2').innerHTML=thRow(['#','Name','Email','Market','Manager','Missing Curriculum']);
    sel('print-roster-body2').innerHTML=secGaps.length
      ? secGaps.sort((a,b)=>(a.Manager||'').localeCompare(b.Manager||''))
          .map((p,i)=>{{
            const missing=[p.AcuteCare!=='Yes'?'Acute Care':null,p.Ambulatory!=='Yes'?'Ambulatory':null,p.Extended!=='Yes'?'Extended Care':null].filter(Boolean).join(', ');
            return tds([i+1,`<b>${{p.FirstName}} ${{p.LastName}}</b>`,p.Email||'—',p.Market||'—',p.Manager||'—',missing]);
          }}).join('')
      : '<tr><td colspan="6" style="color:#999;font-style:italic;padding:10px 8px">No employees in this category</td></tr>';
    doPrint(true);
  }}
