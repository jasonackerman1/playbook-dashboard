...
function setupPrintHeader(title,subtitle) {{
  ...
  const certLabel=sel('f-cert').options[sel('f-cert').selectedIndex].text;
  ...
  const parts=[`Curriculum: ${{certLabel}}`,`Status: ${{statusLabel}}`,`Market: ${{market}}`];
...
function runExport(type) {{
  ...
  if(type==='full') {{
    ...
    var pTotal=filtered.length,pCert=filtered.filter(p=>p[certField]==='Yes').length;
    ...
    var qMap={{}};
    filtered.filter(p=>p[certField]==='Yes'&&p.Date).forEach(p=>{{var q=pFiscalQtr(p.Date);if(q)qMap[q]=(qMap[q]||0)+1;}});
    var qRows=Object.entries(qMap).sort((a,b)=>a[0].localeCompare(b[0]));
    if(!qRows.length) qRows=[['No data','—']];
    var pAcute=filtered.filter(p=>p.AcuteCare==='Yes').length,pAmb=filtered.filter(p=>p.Ambulatory==='Yes').length,pExt=filtered.filter(p=>p.Extended==='Yes').length;
    sel('print-charts').innerHTML=pSection('Certification Pipeline',[['In Progress',pIP],['LMS Complete',pLMS],['Certified',pCert]])+pSection('Certifications by Quarter',qRows)+pSection('Secondary Curriculum',[['Acute Care',pAcute],['Ambulatory',pAmb],['Extended Care',pExt]]);
    sel('print-roster-head').innerHTML=thRow(['#','Name','Market','Job Title','Curriculum Status','Cert Date','Acute Care','Ambulatory','Extended Care','Manager','Email']);
    sel('print-roster-body').innerHTML=filtered.map((p,i)=>{{
      const status=p[certField]==='Yes'?'Certified':p.Complete==='Yes'?'LMS Complete':'In Progress';
      const acute=p.AcuteCare==='Yes'?`Yes (${{p.AcuteDate||''}})`:'No';
      const amb=p.Ambulatory==='Yes'?`Yes (${{p.AmbDate||''}})`:'No';
      const ext=p.Extended==='Yes'?`Yes (${{p.ExtDate||''}})`:'No';
      return tds([i+1,`<b>${{p.FirstName}} ${{p.LastName}}</b>`,p.Market||'—',p.JobTitle||'—',status,p.Date||'—',acute,amb,ext,p.Manager||'—',`<small>${{p.Email||'—'}}</small>`]);
    }}).join('');
    doPrint(false);

  }} else if(type==='not-certified') {{
    const notCert=filtered.filter(p=>p[certField]!=='Yes');
    ...
    sel('ph-desc').textContent=`Employees who have not yet earned ${{sel('f-cert').options[sel('f-cert').selectedIndex].text}} certification, sorted by manager. Use this list to contact employees and their managers to drive completion.`;
    ...
    filtered.forEach(p=>{{
      ...
      mgrMap[k].total++;
      if(p[certField]==='Yes') mgrMap[k].cert++;
    ...
    const almostThere=filtered.filter(p=>p.Complete==='Yes'&&p[certField]!=='Yes');
    const secGaps=filtered.filter(p=>p[certField]==='Yes'&&(p.AcuteCare!=='Yes'||p.Ambulatory!=='Yes'||p.Extended!=='Yes'));
    setupPrintHeader('Action Required — {name}',`Generated: ${{now}}  |  ${{almostThere.length+secGaps.length}} Employees Need Follow-Up`);
    sel('ph-desc').textContent='Employees who have started but not yet completed their certification journey. Each section identifies a different gap and the managers responsible for follow-up.';
    sel('ph-desc').style.display='block';
    sel('print-s1-label').innerHTML=`LMS COMPLETE — AWAITING CERTIFICATION (${{almostThere.length}})<div style="font-weight:400;text-transform:none;letter-spacing:0;font-size:10px;color:#555;font-style:italic;margin-top:4px">These employees have completed the LMS curriculum but have not yet been certified. Follow up to confirm exam status or escalate to completion.</div>`;
    ...
    sel('print-s2').style.display='block';
    sel('print-s2-label').innerHTML=`CERTIFIED — MISSING SECONDARY CURRICULUM (${{secGaps.length}})<div style="font-weight:400;text-transform:none;letter-spacing:0;font-size:10px;color:#555;font-style:italic;margin-top:4px">These employees hold their certification but have not completed all secondary curriculums. Follow up to encourage enrollment in outstanding courses.</div>`;
    sel('print-roster-head2').innerHTML=thRow(['#','Name','Email','Market','Manager','Acute Care','Ambulatory','Extended Care']);
    sel('print-roster-body2').innerHTML=secGaps.length
      ? secGaps.sort((a,b)=>(a.Manager||'').localeCompare(b.Manager||''))
          .map((p,i)=>tds([i+1,`<b>${{p.FirstName}} ${{p.LastName}}</b>`,p.Email||'—',p.Market||'—',p.Manager||'—',
            p.AcuteCare==='Yes'?'&#10003;':'&#10007;',p.Ambulatory==='Yes'?'&#10003;':'&#10007;',p.Extended==='Yes'?'&#10003;':'&#10007;'])).join('')
      : '<tr><td colspan="8" style="color:#999;font-style:italic;padding:10px 8px">No employees in this category</td></tr>';
    doPrint(true);
  }}
}}
...
const INFO_MSGS = {{
  ...
  'subcert':             'Certification counts broken out by curriculum. Healthcare Foundational must be completed first. Acute Care, Ambulatory, and Extended Care are secondary curriculums that build on top of it. Secondary curriculum data is currently in progress.',
  'roster':              'Full list of assigned people with their certification status and pipeline position. Click a name to see job title, market, cert date, secondary curriculum progress, and manager info. Use the View toggle to group by manager.',
  'export':              'Export a printable report of the data currently on screen. Filter first, then export — the report only includes what passes your active filters. Examples: set Certification = "Acute Care" then export for an Acute Care–only report; filter to a region then export for a regional snapshot; hide TLG then export to share with managers. Full Report lists everyone with full detail. Not Certified is a contact list for outreach. Manager Summary rolls up team count and completion % per manager. Action Required flags who is close to fully certified and who still needs the main curriculum.',
}};
...
<div id="print-roster-wrap" style="display:none;">
  ...
  <table class="ptable" id="print-roster-table">
    <thead id="print-roster-head"><tr>
      ...
      <th>Curriculum Status</th><th>Cert Date</th>
      <th>Acute Care</th><th>Ambulatory</th><th>Extended Care</th>
      <th>Manager</th><th>Email</th>
    </tr></thead>
    <tbody id="print-roster-body"></tbody>
  </table>
  <div id="print-s2" style="display:none;margin-top:28px;">
    <div id="print-s2-label" style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:#555;margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid #1a73e8;"></div>
    <table class="ptable">
      <thead id="print-roster-head2"><tr></tr></thead>
      <tbody id="print-roster-body2"></tbody>
    </table>
  </div>
</div>
</body>
