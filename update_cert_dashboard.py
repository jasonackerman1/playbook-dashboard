...
<div class="filters">
  ...
  <span id="cert-date-range-label" style="font-size:11px;color:var(--muted);margin-left:2px;white-space:nowrap;"></span>
  <button class="btn-reset" onclick="resetFilters()">Reset</button>
  <span class="result-count" id="result-count"></span>
</div>
...
function applyFilters(){{
  ...
  var certDates = filtered.map(function(p){{ return p.CertDate; }}).filter(Boolean).sort();
  var rangeLabel = sel("cert-date-range-label");
  if(rangeLabel) {{
    if(certDates.length > 0) {{
      var fmt = function(d){{ var p=d.split('-'); return p[1]+'/'+p[2]+'/'+p[0].slice(2); }};
      rangeLabel.textContent = certDates.length === 1
        ? "(" + fmt(certDates[0]) + ")"
        : "(" + fmt(certDates[0]) + " – " + fmt(certDates[certDates.length-1]) + ")";
    }} else {{
