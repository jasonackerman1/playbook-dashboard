...
<div class="charts">
  ...
  <div class="chart-card">
    <div class="chart-title">Certification Breakdown <span class="info-btn" onclick="showInfo(event,'subcert')">?</span></div>
...
const INFO_MSGS = {{
  ...
  'subcert':             'How many people have earned each certification. Healthcare Foundational is the base — Acute Care, Ambulatory, and Extended Care require it first.',
...
function renderRosterList(){{
  ...
  sel('roster-left').innerHTML = sorted.map(p => {{
    ...
    return `<div class="roster-person" onclick="rosterSelect(this)" data-name="${{fullName}}">
      ...
      <span class="cert-badge ${{isCert?'yes':'no'}}" title="${{count}}/4 certifications">${{isCert?'Certified':'Not Yet'}}</span>
    </div>`;
...
function rosterSelect(el){{
  ...
  sel('roster-right').innerHTML = `
    ...
    <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">
      <div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">Certifications</div>
